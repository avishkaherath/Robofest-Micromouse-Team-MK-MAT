...
queue=[]
#done
def orientation(orient,turning):###whan left turn, substitute 1 from current orientation
    if (turning== 'L'):
        orient-=1
        if (orient==-1):
            orient=3
...
###print error messages seperately
def log(string):
    sys.stderr.write("{}\n".format(string))
#done
...
#done
###recreate flood array according to destination
...
#done.....creating the maze with walls from the centre
def floodFill2(maze):
    for i in range(16):
        for j in range(16):
            maze[i][j]=0
...
#done####fllodfill from current cell,,,update maze
...
def toMove(maze,x,y,xprev,yprev,orient):
    ...
    val= maze[y][x]###current cell number
    prev=0
    ...
    minVal=minVals[0]
    minCell=0
    noMovements=0###number of movements
    for i in minVals:
        if (i!=1000):
            noMovements+=1
...
def main():
    ...
    while True:
        ...
        if short:
            direction= toMove2(flood2,x,y,xprev,yprev,orient)####returns the direction to move
        else:
